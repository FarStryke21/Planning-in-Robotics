...
int calculateHeuristic(int x, int y, int target_x, int target_y) {
    return 5*(abs(x - target_x) + abs(y - target_y));
}
...
void planner(
    ...
    while (!openSet.empty()) {
        ...
        // printf("Current node : (%d, %d) | Address : %p\n" , (*current).x, (*current).y, (void*)&current);
        ...
        for (int dir = 0; dir < NUMOFDIRS; dir++) {
            ...
            if (isValid(new_x, new_y, map, collision_thresh, x_size, y_size)) {
                ...
                g = (*current).g + 1; //2*map[GETMAPINDEX(new_x, new_y, x_size, y_size)];
