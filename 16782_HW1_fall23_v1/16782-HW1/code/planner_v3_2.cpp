...
int calculateHeuristic(int x, int y, int target_x, int target_y, double* map, int collision_thresh, int x_size, int y_size, int target_steps, double* target_traj) {
    ...
    while (!openSet.empty()) {
        ...
        for (int dir = 0; dir < NUMOFDIRS; dir++) {
            ...
            if (isValid(new_x, new_y, map, collision_thresh, x_size, y_size)) {
                ...
                h = calculateHeuristic(new_x, new_y, goalposeX, goalposeY, map, collision_thresh, x_size, y_size, target_steps, target_traj);
